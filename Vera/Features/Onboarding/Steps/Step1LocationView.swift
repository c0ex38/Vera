...
struct Step1LocationView: View {
    ...
    var body: some View {
        VStack(spacing: 24) {
            ...
            if !preferences.savedDistrictID.isEmpty {
                VStack(spacing: 8) {
                    ...
                    Text(preferences.savedLocationName).font(.headline).foregroundColor(.themePrimary)
        ...
        .onChange(of: preferences.savedDistrictID) { _, newValue in
            if !newValue.isEmpty {
                // Konum başarıyla kaydedildi, bir sonraki adıma otomatik geç
